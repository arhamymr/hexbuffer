...
function ScriptAnalyzeForm({ params, updateParam }: ActionParams) {
  return (
    <>
      <SelectField label="Source field" value={params.sourceField} fallback="body" onChange={(v) => updateParam('sourceField', v)} options={REQUEST_SOURCE_OPTIONS} />
      <BooleanField label="Include inline scripts" value={params.includeInline} fallback="true" onChange={(v) => updateParam('includeInline', v)} />
      <BooleanField label="Include external scripts" value={params.includeExternal} fallback="true" onChange={(v) => updateParam('includeExternal', v)} />
    </>
  );
}
...
const ACTION_CONFIG_MAP: Record<string, React.FC<ActionParams>> = {
  ...
  'action:script-analyze': ScriptAnalyzeForm,
};
