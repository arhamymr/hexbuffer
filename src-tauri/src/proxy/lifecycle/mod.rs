        ...
        if ctx.req_method != "CONNECT" {
            if let Some(mock_state) = self.app_handle.try_state::<crate::commands::mock_forge::MockForgeState>() {
                ...
                let host_str = if host_str_raw.is_empty() {
                    ctx.req_headers.get("host").map(|h| h.split(':').next().unwrap_or(h)).unwrap_or("")
                } else {
                    host_str_raw
                };
                
                let path_str = uri_parsed.as_ref().map(|u| u.path()).unwrap_or_else(|| {
                    ctx.req_uri.split('?').next().unwrap_or("/")
                });
                
                let query_map: HashMap<String, String> = if let Some(ref u) = uri_parsed {
                    u.query_pairs().into_owned().collect()
                } else {
                    if let Some(pos) = ctx.req_uri.find('?') {
