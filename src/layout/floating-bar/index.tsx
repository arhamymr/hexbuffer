...
function SortableDockItem({ item, active, dragActive }: { item: NavItem; active: boolean; dragActive: React.RefObject<boolean> }) {
  const {
    attributes,
    listeners,
    setNodeRef,
    ...
    <Tooltip>
      <TooltipTrigger asChild>
        <Link
          ...
          className={
            item.iconImage
              ...
              : cn(
                  'relative flex size-7 items-center justify-center rounded-sm text-muted-foreground transition-all hover:bg-muted/80 hover:text-foreground hover:scale-125 touch-none group/dock-item',
        ...
        >
          ...
          {item.iconImage ? (
            <img src={item.iconImage} alt="" className="size-7 object-cover rounded-sm" />
          ) : (
            <item.icon className="size-4" />
          )}
        </Link>
      </TooltipTrigger>
      <TooltipContent side="top" sideOffset={12}>{item.label}</TooltipContent>
    </Tooltip>
  );
}
...
export function AppSidebar() {
  ...
  function handleDragEnd(event: DragEndEvent) {
    ...
    setTimeout(() => { dragActive.current = false; }, 100);
  }
              ...
              <Link
                ...
                className={
                  overviewItem.iconImage
                    ...
                    : cn(
                        'relative flex size-7 items-center justify-center rounded-sm text-muted-foreground transition-all hover:bg-muted/80 hover:text-foreground hover:scale-125',
                        isNavItemActive(overviewItem) && 'bg-primary/15 text-primary',
                      )
                }
              >
                {overviewItem.iconImage ? (
                  <img src={overviewItem.iconImage} alt="" className="size-7 object-cover rounded-sm" />
                ) : (
                  <overviewItem.icon className="size-4" />
                )}
              </Link>
            </TooltipTrigger>
            <TooltipContent side="top" sideOffset={12}>{overviewItem.label}</TooltipContent>
          </Tooltip>
        )}

        {/* Pinned items — sortable via drag-and-drop */}
        <DndContext sensors={sensors} collisionDetection={closestCenter} onDragStart={() => { dragActive.current = true; }} onDragEnd={handleDragEnd}>
          <SortableContext items={pinnedDockItems.map((i) => i.href)} strategy={horizontalListSortingStrategy}>
            {pinnedDockItems.map((item) => (
              <SortableDockItem key={item.href} item={item} active={isNavItemActive(item)} dragActive={dragActive} />
            ))}
          </SortableContext>
        </DndContext>
