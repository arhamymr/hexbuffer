...
export function createWorkspace(name?: string): string {
  return useRepeaterStore.getState().createWorkspace(name);
}
...
export async function createCollection(workspaceId: string, name: string): Promise<string> {
  return await useCollectionsStore.getState().createStash(name, workspaceId);
}
...
export async function createFolder(parentId: string, name: string): Promise<string> {
  return await useCollectionsStore.getState().createStash(name, parentId);
}
