...
impl Database {
    ...
    pub fn delete_collaborator_server(&self, id: &str) -> SqlResult<()> {
        let conn = self.conn.lock().unwrap();
        conn.execute("DELETE FROM collaborator_servers WHERE id = ?1", params![id])?;
        Ok(())
    }
    ...
    pub fn list_collaborator_payloads(&self, server_id: Option<&str>) -> SqlResult<Vec<CollaboratorPayload>> {
        let conn = self.conn.lock().unwrap();
    ...
    pub fn delete_collaborator_payload(&self, id: &str) -> SqlResult<()> {
        let conn = self.conn.lock().unwrap();
        conn.execute("DELETE FROM collaborator_payloads WHERE id = ?1", params![id])?;
        Ok(())
    }

    pub fn increment_collaborator_payload_interactions(&self, id: &str, count: i64) -> SqlResult<()> {
        let conn = self.conn.lock().unwrap();
        conn.execute(
    ...
    pub fn get_collaborator_interaction(&self, id: &str) -> SqlResult<Option<CollaboratorInteraction>> {
        let conn = self.conn.lock().unwrap();
        conn.query_row(
    ...
    pub fn get_collaborator_dashboard_stats(&self) -> SqlResult<CollaboratorDashboardStats> {
        ...
        let active_payloads: i64 = conn.query_row(
            "SELECT COUNT(*) FROM collaborator_payloads WHERE status = 'active'", [], |r| r.get(0),
        )?;
        let interactions_today: i64 = conn.query_row(
            "SELECT COUNT(*) FROM collaborator_interactions WHERE timestamp >= ?1", params![today], |r| r.get(0),
        )?;
        let dns_events: i64 = conn.query_row(
            "SELECT COUNT(*) FROM collaborator_interactions WHERE interaction_type = 'dns'", [], |r| r.get(0),
        )?;
        let http_events: i64 = conn.query_row(
            "SELECT COUNT(*) FROM collaborator_interactions WHERE interaction_type = 'http'", [], |r| r.get(0),
        )?;
        let https_events: i64 = conn.query_row(
            "SELECT COUNT(*) FROM collaborator_interactions WHERE interaction_type = 'https'", [], |r| r.get(0),
        )?;
        let last_callback: Option<String> = conn.query_row(
            "SELECT MAX(timestamp) FROM collaborator_interactions", [], |r| r.get(0),
        )?;
        let connected_servers: i64 = conn.query_row(
            "SELECT COUNT(*) FROM collaborator_servers WHERE status = 'connected'", [], |r| r.get(0),
        )?;
