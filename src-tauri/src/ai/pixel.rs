...
use super::types::{
    AiEnginePixelMessage, PixelGenerationRequest, PixelGenerationResponse,
};

pub async fn generate_pixel_matrix_impl(
    app: AppHandle,
    ...
    let response = tauri::async_runtime::spawn_blocking(move || {
        run_pixel_generate_engine(&app, &settings.provider, &settings.model, &api_key, &request)
    })
    .await
...
fn run_pixel_generate_engine(
    ...
    let palette_json = serde_json::to_string(&request.palette).map_err(|e| e.to_string())?;

    let sidecar_command = app
        .shell()
        .sidecar("ai-engine")
        .map_err(|error| format!("Failed to prepare AI engine sidecar: {}", error))?
        .env("HEXBUFFER_AI_ENGINE_MODE", "pixel-generate")
        .env("HEXBUFFER_PIXEL_PROMPT", request.prompt.clone())
        .env("HEXBUFFER_PIXEL_WIDTH", request.width.to_string())
        .env("HEXBUFFER_PIXEL_HEIGHT", request.height.to_string())
        .env("HEXBUFFER_PIXEL_PALETTE", palette_json)
    ...
    let mut response: Option<PixelGenerationResponse> = None;
    ...
    for line in reader.lines() {
        ...
        let message: AiEnginePixelMessage = serde_json::from_str(&line)
            .map_err(|error| format!("Invalid AI engine message: {} ({})", line, error))?;

        match message.message_type.as_str() {
            "pixel_generate_finished" => {
                if let Some(data) = message.data {
                    response = Some(PixelGenerationResponse {
            ...
            "pixel_generate_failed" => {
                failed = Some(
                    message
                        .message
                        .unwrap_or_else(|| "AI pixel generation failed".to_string()),
                );
            }
            _ => {}
        }
    ...
    response.ok_or_else(|| "AI engine did not return pixel data".to_string())
}
