(function() {
  ...
  // the current `window.__apprecon_annotation_mode`. Silently returning here would
  ...
  var __apprecon_existing_layer = document.getElementById('__apprecon_annotation_layer');
  if (__apprecon_existing_layer) {
    var __apprecon_pending_mode = window.__apprecon_annotation_mode;
    var __apprecon_pending_tab_id = window.__apprecon_annotation_tab_id;
    ...
    var __apprecon_stale = document.getElementById('__apprecon_annotation_layer');
    while (__apprecon_stale) {
      __apprecon_stale.remove();
      __apprecon_stale = document.getElementById('__apprecon_annotation_layer');
    }
    window.__apprecon_annotation_mode = __apprecon_pending_mode;
    window.__apprecon_annotation_tab_id = __apprecon_pending_tab_id;
  }

  var OVERLAY_ID = '__apprecon_annotation_layer';
  var RECT_ID = '__apprecon_annotation_rect';
  var HIGHLIGHT_ID = '__apprecon_annotation_highlight';
  ...
  var mode = window.__apprecon_annotation_mode === 'select' ? 'select' : 'draw';
  ...
  function ensureHighlight() {
    ...
    getMountRoot().appendChild(highlightEl);
    return highlightEl;
  }
  ...
  function collectComputedStyles(element) {
    ...
    for (var index = 0; index < STYLE_ALLOWLIST.length; index += 1) {
      var propertyName = STYLE_ALLOWLIST[index];
      var propertyValue = computed.getPropertyValue(propertyName);
      if (!propertyValue) continue;

      styles[propertyName] = stripControlChars(propertyValue.trim());
    }
  ...
  function captureElementPayload(element) {
    ...
    return {
      tabId: window.__apprecon_annotation_tab_id || '',
  ...
  function onMouseDown(e) {
    ...
    getMountRoot().appendChild(rectEl);
  }
  ...
  // --- Marker System ---
  var MARKER_CONTAINER_ID = '__apprecon_marker_container';
  var MARKER_CLASS = '__apprecon_marker';
  ...
  function ensureMarkerContainer() {
    ...
    getMountRoot().appendChild(markerContainer);
  ...
  function createMarkerElement(data, selectedId) {
    ...
    if (data.id === selectedId) {
      el.classList.add('__apprecon_marker_selected');
    }
  ...
  if (!document.getElementById('__apprecon_marker_styles')) {
    var style = document.createElement('style');
    style.id = '__apprecon_marker_styles';
    style.textContent = [
      '.' + MARKER_CLASS + ' {',
      '  transition: transform 0.15s ease, background 0.1s ease, border-color 0.1s ease;',
      '}',
      '.' + MARKER_CLASS + '.__apprecon_marker_selected {',
      '  background: #3b82f6;',
      '  border-color: #ffffff;',
      '  transform: scale(1.2);',
  ...
  window.__apprecon_render_markers = function(annotations, selectedId) {
    window.__apprecon_remove_markers();
  ...
  window.__apprecon_update_marker_selection = function(selectedId) {
    ...
    for (var i = 0; i < markers.length; i++) {
      ...
      if (isSelected) {
        m.classList.add('__apprecon_marker_selected');
      } else {
        m.classList.remove('__apprecon_marker_selected');
      }
    }
  };

  window.__apprecon_remove_markers = function() {
    stopMarkerTracking();
    if (markerContainer) {
  ...
  window.__apprecon_remove_annotation_overlay = function() {
    window.__apprecon_remove_markers();
    var markerStylesEl = document.getElementById('__apprecon_marker_styles');
    if (markerStylesEl) {
    ...
    delete window.__apprecon_remove_annotation_overlay;
    delete window.__apprecon_annotation_tab_id;
    delete window.__apprecon_annotation_mode;
  };
