...
export const getBrowserTabsDef = {
  description:
  ...
  execute: async ({ sessionId, status }, ctx) => {
    ctx.emitAction({
      action: 'get_browser_tabs',
      payload: { sessionId, status },
    });

    const crawlSessions = ctx.redactedContext.crawlSessions || [];
    ...
    if (ctx.redactedContext.latestCrawl?.session?.id === targetId) {
      pages = ctx.redactedContext.latestCrawl.pages || [];
      session = ctx.redactedContext.latestCrawl.session;
    }
